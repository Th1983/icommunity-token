...
contract IComFarm is ReentrancyGuard, Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 lpTokenBalance;     // LP token balance
    ...
    function pending(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 lpSupply = pool.lpTokenBalance; // .balanceOf(address(this));
    ...
    function depositFromVesting(uint256 _pid, address _sender, uint256 _amount) public onlyOwner nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (user.amount > 0) {
            uint256 pendingAmount = (user.amount * pool.accERC20PerShare / 1e36) - user.rewardDebt;
            if(pendingAmount > 0) {
                safeErc20Transfer(_sender, pendingAmount);
            }
        }
        if (_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
